...
//!
//! Use feature `encrypted_private_key` for an example with a private key encoded with a password
#![no_std]
#![no_main]
...
fn main() -> ! {
    ...
    let certificates = Certificates {
        ca_chain: Some(X509::pem(CERT.as_bytes()).unwrap()),
        certificate: Some(X509::pem(CLIENT_CERT.as_bytes()).unwrap()),
        private_key: Some(X509::pem(PRIVATE_KEY.as_bytes()).unwrap()),
        password: None,
    };
    ...
    let certificates = Certificates {
        ca_chain: Some(X509::pem(CERT.as_bytes()).unwrap()),
        certificate: Some(X509::pem(CLIENT_CERT_WITH_PASSWORD.as_bytes()).unwrap()),
        private_key: Some(X509::pem(PRIVATE_KEY_WITH_PASSWORD.as_bytes()).unwrap()),
